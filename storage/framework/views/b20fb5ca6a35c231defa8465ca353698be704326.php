...
<section class="content-header">
    <h1><?php echo e($title, false); ?> <small>(<?php if($transaction->type == 'sales_order'): ?> <?php echo app('translator')->getFromJson('restaurant.order_no'); ?> <?php else: ?> <?php echo app('translator')->getFromJson('sale.invoice_no'); ?> <?php endif; ?>: <span class="text-success">#<?php echo e($transaction->invoice_no, false); ?>)</span></small></h1>
</section>
<!-- Main content -->
...
<input type="hidden" id="item_addition_method" value="<?php echo e($business_details->item_addition_method, false); ?>">
	<?php echo Form::open(['url' => action('SellPosController@update', ['id' => $transaction->id ]), 'method' => 'put', 'id' => 'edit_sell_form', 'files' => true ]); ?>
		...
		<div class="col-md-12 col-sm-12">
			<?php $__env->startComponent('components.widget', ['class' => 'box-solid']); ?>
				...
				<div class="<?php if(!empty($commission_agent)): ?> col-sm-3 <?php else: ?> col-sm-4 <?php endif; ?>">
					...
					<small>
						<strong>
							<?php echo app('translator')->getFromJson('lang_v1.billing_address'); ?>:
						</strong>
						...
						<br>
						<strong>
							<?php echo app('translator')->getFromJson('lang_v1.shipping_address'); ?>:
						</strong>
				...
				<?php else: ?>
					<div class="<?php if(!empty($commission_agent)): ?> col-sm-3 <?php else: ?> col-sm-4 <?php endif; ?>">
						<div class="form-group">
							...
							<?php echo Form::select('status', $statuses, $status, ['class' => 'form-control select2', 'placeholder' => __('messages.please_select'), 'required']);; ?>
				...
				<div class="col-sm-3">
					<div class="form-group">
						...
						<?php echo Form::text('invoice_no', $transaction->invoice_no, ['class' => 'form-control', 'placeholder' => $transaction->type == 'sales_order' ? __('restaurant.order_no'): __('sale.invoice_no')]);; ?>
		        ...
		        <div class="col-sm-3">
	                <div class="form-group">
	                    ...
	                    <p class="help-block"><?php echo app('translator')->getFromJson('purchase.max_file_size', ['size' => (config('constants.document_size_limit') / 1000000)]); ?>
		        ...
		        <?php if((!empty($pos_settings['enable_sales_order']) && $transaction->type != 'sales_order') || $is_order_request_enabled): ?>
					<div class="col-sm-3">
						<div class="form-group">
							...
							<?php echo Form::select('sales_order_ids[]', $sales_orders, $transaction->sales_order_ids, ['class' => 'form-control select2 not_loaded', 'multiple', 'id' => 'sales_order_ids']);; ?>
					...
					<table class="table table-condensed table-bordered table-striped table-responsive" id="pos_table">
						<thead>
							<tr>
								...
								<th <?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('edit_product_discount_from_sale_screen')): ?> hide <?php endif; ?>>
									<?php echo app('translator')->getFromJson('receipt.discount'); ?>
								</th>
								...
								<?php if(!empty($common_settings['enable_product_warranty'])): ?>
									<th><?php echo app('translator')->getFromJson('lang_v1.warranty'); ?></th>
								<?php endif; ?>
								<th class="text-center">
						...
						<tbody>
							<?php $__currentLoopData = $sell_details; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $sell_line): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
								<?php echo $__env->make('sale_pos.product_row', ['product' => $sell_line, 'row_count' => $loop->index, 'tax_dropdown' => $taxes, 'sub_units' => !empty($sell_line->unit_details) ? $sell_line->unit_details : [], 'action' => 'edit', 'is_direct_sell' => true, 'so_line' => $sell_line->so_line, 'is_sales_order' => $transaction->type == 'sales_order'], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
			...
			<?php $__env->startComponent('components.widget', ['class' => 'box-solid']); ?>
			    ...
			    <div class="clearfix"></div>
			    <div class="col-md-12 well well-sm bg-light-gray <?php if(session('business.enable_rp') != 1 || $transaction->type == 'sales_order'): ?> hide <?php endif; ?>">
				...
				<?php $__env->startComponent('components.widget', ['class' => 'box-solid', 'title' => __('lang_v1.export')]); ?>
					<div class="col-md-12 mb-12">
		                <div class="form-check">
		                    ...
		                    <label class="form-check-label" for="is_export"><?php echo app('translator')->getFromJson('lang_v1.is_export'); ?></label>
		                </div>
		            </div>
			        <?php
	                	$i = 1;
	...
	<?php if($is_enabled_download_pdf && $transaction->type != 'sales_order'): ?>
		<?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('sell.payments')): ?>
			<?php $__env->startComponent('components.widget', ['class' => 'box-solid', 'title' => __('purchase.add_payment')]); ?>
				<div class="well row">
					<div class="col-md-6">
						<div class="form-group">
                ...
                ?>
							<div class="input-group">
								...
								<?php echo Form::select("prefer_payment_method", $payment_types, $transaction->prefer_payment_method, ['class' => 'form-control','style' => 'width:100%;']);; ?>
                ...
                ?>
							<div class="input-group">
								...
								<?php echo Form::select("prefer_payment_account", $accounts, $transaction->prefer_payment_account, ['class' => 'form-control','style' => 'width:100%;']);; ?>
	...
	<?php if (app(\Illuminate\Contracts\Auth\Access\Gate::class)->check('sell.payments')): ?>
		<?php $__env->startComponent('components.widget', ['class' => 'box-solid', 'title' => __('purchase.add_payment')]); ?>
			...
			<?php $__currentLoopData = $payment_lines; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $payment_line): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>			
				...
				<?php echo $__env->make('sale_pos.partials.payment_row_form', ['row_index' => $loop->index, 'show_date' => true, 'payment_line' => $payment_line], \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?>
			<?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
			</div>
			...
			<div class="col-md-12">
        		<hr>
        		<strong>
        			<?php echo app('translator')->getFromJson('lang_v1.change_return'); ?>:
        		</strong>
        		<br/>
	...
	<div class="row">
		<div class="col-md-12 text-center">
	    	...
	    	<button type="button" class="btn btn-primary btn-big" id="submit-sell"><?php echo app('translator')->getFromJson('messages.update'); ?></button>
	    	<button type="button" id="save-and-print" class="btn btn-success btn-big"><?php echo app('translator')->getFromJson('lang_v1.update_and_print'); ?></button>
	    </div>
	</div>
	<?php if(in_array('subscription', $enabled_modules)): ?>
...
<?php echo $__env->make('layouts.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH /home/u997099361/domains/nacionalcode.org/public_html/possaas/resources/views/sell/edit.blade.php ENDPATH**/ ?>