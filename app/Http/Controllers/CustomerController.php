    ...
    public function index()
        ...
        
        {
           ...
           $business_id = request()->session()->get('user.business_id');
            ...
            $customers = Customer::where('customers.business_id', $business_id )->get();
...
public function index()
{
    ...
    if (request()->ajax()) {
        ...
        $customer = Customer::where('customers.business_id', $business_id)
                                //->leftjoin('selling_price_groups as spg', 'spg.id', '=', 'customer_groups.selling_price_group_id')
                            ->select(['customers.name', 'customers.contact', 'customers.email',  'customers.balance','customers.id']);

        return Datatables::of($customer)
                ->addColumn(
                    'action',
                    '@can("customer.update")
                        <button data-href="{{action(\'CustomerController@edit\', [$id])}}" class="btn btn-xs btn-primary edit_customer_button"><i class="glyphicon glyphicon-edit"></i> @lang("messages.edit")</button>
                    &nbsp;
                    @endcan

                    @can("customer.delete")
                        <button data-href="{{action(\'CustomerController@destroy\', [$id])}}" class="btn btn-xs btn-danger delete_customer_button"><i class="glyphicon glyphicon-trash"></i> @lang("messages.delete")</button>
                    @endcan'
                )
    ...
    public function update(Request $request, $id)
    {
        ...
        if (request()->ajax()) {
            try {
                $input = $request->only(['name','contact','email','balance']);
                $business_id = $request->session()->get('user.business_id');

               // $input['amount'] = !empty($input['amount']) ? $this->commonUtil->num_uf($input['amount']) : 0;

                $customer = Customer::where('business_id', $business_id)->findOrFail($id);
    ...
    public function update_antes(Request $request, Customer $customer)
    {
